...
namespace UProveCrypto
{
    ...
    public static class ProtocolHelper
    {
        ...
        public static FieldZqElement ComputeXt(IssuerParameters ip, byte[] TI, bool supportDevice)
        {
        ...
        public static FieldZqElement ComputeXi(IssuerParameters ip, int i, byte[] A)
        {
            if (i < 0)
            {
                throw new ArgumentException("index i can't be negative: " + i);
            }
        ...
        internal static FieldZqElement GenerateChallenge(IssuerParameters ip, UProveToken upt, byte[] a, int pseudonymIndex, byte[] ap, GroupElement Ps, byte[] m, byte[] md, int[] disclosed, FieldZqElement[] disclosedX, int[] committed, CommitmentValues[] commitments, out byte[] cp)
        {
            ...
            // compute prover challenge contribution
            HashFunction hash = ip.HashFunction;
        ...
        private const int ScopeElementIndex = 0;
        /// <summary>
