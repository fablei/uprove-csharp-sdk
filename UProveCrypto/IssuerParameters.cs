...
namespace UProveCrypto
{
    ...
    /// </summary>
    [DataContract]
    public class IssuerParameters : IParametrizedDeserialization
    {
        private byte[] uidp;
        private Group group;
        private string uidh;
        ...
        /// </summary>
        public T Deserialize<T>(string serializedString) where T : IParametrizedDeserialization
        {
            try
            {
                T t = serializer.FromJson<T>(serializedString);
                t.FinishDeserialization(this);
                return t;
            }
            catch
            {
